...
class AMapLocationPoint extends NSObject with NSCoding, NSCopying {
  ...
  Future<double> get_latitude() async {
    final result = await _channel.invokeMethod("AMapLocationPoint::get_latitude", {'refId': refId});
    return result;
  }
  
  Future<double> get_longitude() async {
    final result = await _channel.invokeMethod("AMapLocationPoint::get_longitude", {'refId': refId});
    return result;
  }
  ...
  Future<void> set_latitude(double latitude) async {
    await _channel.invokeMethod('AMapLocationPoint::set_latitude', {'refId': refId, "latitude": latitude});
  
  
  }
  
  Future<void> set_longitude(double longitude) async {
    await _channel.invokeMethod('AMapLocationPoint::set_longitude', {'refId': refId, "longitude": longitude});
  ...
  // 生成方法们
  static Future<AMapLocationPoint> locationWithLatitude(double lat, double lon) async {
    // 日志打印
    print('fluttify-dart: AMapLocationPoint::locationWithLatitude([\'lat\':$lat, \'lon\':$lon])');
  
    // 调用原生方法
    final result = await _channel.invokeMethod('AMapLocationPoint::locationWithLatitudeLongitude', {"lat": lat, "lon": lon});
